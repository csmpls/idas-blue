...
func (user *User) NewPostFromContainerJson(rawJson []byte) (*Post, error) {
  ...
  post := user.NewPost(container.Post.Title, container.Post.IsEncrypted)
  return post, nil
}
 ...
 */
func (user *User) NewPost(title string, encrypted bool) *Post {
  post := &Post{}
  post.InitNew(title, user.FullPath, user.Id, user.Name, user.Avatar, encrypted, user.IsMyUser)
  return post
}
