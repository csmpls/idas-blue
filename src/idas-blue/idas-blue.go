...
var DIR = func() string {
  ...
  return path.Join(dir, "idas-blue")
}()
  ...
  web.Get("/api/posts", func(ctx *web.Context) string {
    ...
    if userId != "" {
      ...
      if err != nil || len(allPosts.Posts) < 1 {
        ...
        post := &store.Post{}
        post.InitNew("Welcome to idas blue. To start, create a new profile by clicking 'My Profile' on the left.", "none", "none", "Welcome", "", false, false)
        post.Id = "none"
